      ...
      <div className="mt-10 grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="col-span-1">
          {history.length > 0 ? (
              ...
              <ul className="space-y-2">
                {history.map((file, index) => (
                  <li key={index} className="p-2 bg-base-200 rounded-lg shadow-sm flex flex-col">
                    ...
                    <span>Media Type: <strong>{file.type}</strong></span> {/* Display media type */}
                    <span>Result: <strong>{file.result}</strong></span>
                    <span>Confidence: <strong>{file.confidence}%</strong></span>
                    <span>
                      Emotion Analysis:{" "}
                      <strong>
                        {file.type === 'video' && file.emotion_result
                          ? `${file.emotion_result.emotion} (Score: ${file.emotion_result.score})`
                          : 'N/A'}
                      </strong>
                    </span>
                  </li>
            ...
            <div>
              <img className="w-full ring-2 ring-base-300 max-w-none rounded-xl shadow-xl" src={myImage} alt="Detection" />
            </div>
          )}
        </div>
